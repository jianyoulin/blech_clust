...
raw_car_trace = np.load('common_average_reference.npy')
car_group = np.load('group_assignments.npy')
car_trace = raw_car_trace[int(car_group[electrode_num]), :]
...
filt_car_trace = get_filtered_electrode(car_trace, freq = [bandpass_lower_cutoff, bandpass_upper_cutoff], 
...
test_car_trace = np.reshape(filt_car_trace[:int(sampling_rate)*int(len(filt_car_trace)/sampling_rate)], (-1, int(sampling_rate)))
...
fig = plt.figure()
plt.plot(np.arange(test_el.shape[0]), np.mean(test_el, axis = 1), alpha=0.5)
plt.plot(np.arange(test_car_trace.shape[0]), np.mean(test_car_trace, axis=1), 'r--', alpha=0.5)
...
plot_el = np.mean(filt_el.reshape(int(sampling_rate//1000), -1), axis=0) # convert to ms scale
plot_car_trace = np.mean(filt_car_trace.reshape(int(sampling_rate//1000), -1), axis=0)

fig, axes = plt.subplots(nrows = 10, sharex='col', sharey='row', squeeze = False, figsize=(10, 10))
print(plot_el.shape[0], plot_car_trace.shape[0])
idx = np.random.choice(np.arange(plot_car_trace.shape[0])[0:-60000], size = 10, replace=False)
for i in range(10):
    axes[i,0].plot(np.arange(1000), plot_el[idx[i]:idx[i]+1000], label = f'{np.round(idx[i]/60000, 2)}min', alpha=0.5)
    axes[i,0].plot(np.arange(1000), plot_car_trace[idx[i]:idx[i]+1000], 'r--', label='CAR', alpha=0.3)

    #axes[i,0].plot(np.arange(1000), np.mean(plot_el, axis = 1)[idx[i]:idx[i]+1000], alpha=0.5)
    #axes[i,0].plot(np.arange(1000), np.mean(test_car_trace, axis=1)[idx[i]:idx[i]+1000], 'r--', alpha=0.5)
    if i == 9:
        axes[i,0].set(xlabel = '1sec recording block from the legend time')
    if i == 4:
        axes[i,0].set(ylabel = 'Average voltage recorded per sec (microvolts)')
    axes[i,0].legend(loc='best')
plt.suptitle('Examplary traces')
plt.tight_layout()
fig.savefig('./Plots/%i/Plots/example_traces.png' % electrode_num, bbox_inches='tight')
plt.close("all")
...
del filt_el, test_el, plot_el, plot_car_trace
