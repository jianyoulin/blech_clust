...
def dejitter(slices, spike_times, spike_snapshot = [0.5, 1.0], sampling_rate = 30000.0):
    ...
    for i in range(len(slices)):
        ...
        # Only accept spikes if the interpolated minimum has shifted by less than 1/10th of a ms 
        # (3 samples for a 30kHz recording, 30 samples after interpolation)
        # If minimum hasn't shifted at all, then minimum - 5ms should be equal to zero 
        # (because we sliced out 5 ms before the minimum in extract_waveforms())
        ...
        if np.abs(minimum - int((spike_snapshot[0] + 0.1)*(sampling_rate/100.0))) <= int(10.0*(sampling_rate/10000.0)):
            if len(ynew[minimum - before*10 : minimum + after*10]) == int((before+after)*10):
                slices_dejittered.append(ynew[minimum - before*10 : minimum + after*10])
                spike_times_dejittered.append(spike_times[i])
            else:
                drop_waveforms = drop_waveforms + 1
    print(f'dropped waveforms = {drop_waveforms} out of {len(slices_dejittered)+drop_waveforms}')
