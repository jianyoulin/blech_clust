...
# get trial_num of each digin
trial_nums = [trains_dig_in[i].spike_array[:].shape[0] for i in range(len(trains_dig_in))]
for d_idx, dig_in in enumerate(trains_dig_in):
    ...
    for unit in range(trial_avg_spike_array.shape[0]):
        ...
        plt.ylim(-1, np.max(trial_nums)+1)
